...
export function MeatManagement() {
  ...
  const handleSubmit = async (values: typeof form.values) => {
    try {
      if (values.id) {
          ...
          meats.map((m) =>
            m.id === meatId
              ? {
                  ...
                  meatType: values.meatType as MeatType,
                }
              : m
          )
        );
        notifications.show({
          title: "Success!",
          message: `Meat "${values.description}" has been updated.`,
          color: "green",
        });
      } else {
        const meat = values as Meat;
        ...
        notifications.show({
          title: "Success!",
          message: `Meat "${values.description}" has been added.`,
          color: "green",
        });
      }
      closeAddEditModal();
      ...
      notifications.show({
        title: "Erro!",
        message: `${
          error?.message || "Erro ao tentar criar ou atualizar uma carne"
        }`,
        color: "red",
      });
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      notifications.show({
        title: "Deleted!",
        message: `The meat "${meatToDelete?.description}" has been removed.`,
        color: "green",
      });
    } catch (error) {
      notifications.show({
        title: "Erro ao tentar deletar a carne!",
        message: `${error}`,
        color: "red",
      });
    }
    ...
    <Table.Tr key={meat.id}>
      ...
      <Table.Td>
        <Group gap="xs" wrap="nowrap">
          <Tooltip label="Edit Meat">
            <ActionIcon color="blue" onClick={() => handleOpenEditModal(meat)}>
              <IconPencil size="1rem" />
            </ActionIcon>
          </Tooltip>
          <Tooltip
            label={
              (meat?.numberAssociatedOrders || 0) > 0
                ? `Cannot delete, has ${
                    meat?.numberAssociatedOrders || 0
                  } associated orders`
                : "Delete Meat"
            }
            multiline
          ...
          >
            <div>
              <ActionIcon
                color="red"
                disabled={(meat?.numberAssociatedOrders || 0) > 0}
                onClick={() => openDeleteConfirmation(meat)}
              >
                <IconTrash size="1rem" />
              </ActionIcon>
            </div>
  ...
  return (
    <>
        ...
        >
          ...
          <Table.Tbody>
            {tableRows.length > 0 ? (
              ...
              <Table.Tr>
                <Table.Td colSpan={4}>
                  <Text py="xl">No meats found.</Text>
                </Table.Td>
              </Table.Tr>
            )}
          </Table.Tbody>
      ...
      <Modal
        ...
        title={
          <Title order={3}>
            {form.values.id ? "Edit Meat" : "Add New Meat"}
          </Title>
        }
        centered
      >
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stack>
            ...
            <Group align="right" mt="md">
              ...
              <Button type="submit">
                {form.values.id ? "Update Meat" : "Save Meat"}
              </Button>
            </Group>
          </Stack>
        </form>
      ...
      <Modal
        ...
        onClose={() => setDeleteModalState({ isOpen: false, meatId: null })}
      ...
      >
        <Stack>
          <Text>
            Are you sure you want to delete this meat? This action cannot be
            undone.
          </Text>
          <Group align="right" mt="md">
            <Button
